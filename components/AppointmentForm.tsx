...
const AppointmentForm: React.FC<AppointmentFormProps> = ({ isOpen, onClose, onSave, appointment, t }) => {
  ...
  const handleShareContact = async () => {
    const clinicDetails = {
        title: 'DentalDash Clinic Contact',
        text: `Hello! Here are the contact details for DentalDash Clinic:\nPhone: 07507816500\nLooking forward to seeing you.`,
    };
    ...
    if (navigator.share) {
        try {
            ...
            alert('Could not share contact details.');
        }
    } else {
        navigator.clipboard.writeText(clinicDetails.text)
            .then(() => alert('Contact details copied to clipboard! You can now paste it to send to the patient.'))
            .catch(err => {
                console.error('Failed to copy text: ', err);
                alert('Could not copy contact details. Please copy them manually.');
            });
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave({ ...formData, id: appointment?.id || 'new' });
    onClose();
  };
  
  const InputField: React.FC<{ label: string; name: string; type?: string; value?: any; onChange?: any; required?: boolean; children?: React.ReactNode}> = ({label, name, type="text", value, onChange, required=false, children}) => (
      <div>
