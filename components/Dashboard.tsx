          ...
          <h3 className="text-xl font-bold mb-4">{t('last7DaysFinancials')}</h3>
           <div style={{ width: '100%', height: 300 }}>
            <ResponsiveContainer>
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey={t('income')} fill="#38bdf8" />
                <Bar dataKey={t('expenses')} fill="#f472b6" />
                </BarChart>
            </ResponsiveContainer>
           </div>
        </div>
            ...
            {todaysAppointments.length > 0 ? todaysAppointments.map(apt => (
              <div key={apt.id} onClick={() => onSelectAppointment(apt)} className="p-3 bg-slate-50 rounded-lg cursor-pointer hover:bg-slate-100">
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-sm font-mono bg-sky-100 text-sky-700 px-2 py-1 rounded">{apt.dateTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                </div>
